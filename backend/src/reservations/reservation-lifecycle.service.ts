...
export class ReservationLifecycleService {
  ...
  // ─────────────────────────────────────────────
  validateOwnership(reservationTargetUserId: string, callerUserId: string, callerRole: Role): void {
    if (callerRole === 'ADMIN') return;

    if (reservationTargetUserId !== callerUserId) {
      throw new ForbiddenException('You do not have permission to access or modify this reservation');
    }
  }
